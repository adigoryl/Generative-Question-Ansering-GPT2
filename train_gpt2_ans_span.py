...
def load_json_dataset(dataset_path, tokenizer):
    ...
    for i in range(data['data'].__len__()):
        ...
        story = tokenize_and_encode(data['data'][i]['story'], tokenizer)
        a = []
        q = []
        span_idxs = []

        for idx in range(0, data['data'][i]['answers'].__len__()):

            try:
                span_start = data['data'][i]['answers'][idx]['span_start']
                span_end = data['data'][i]['answers'][idx]['span_end']
                # story_len = len(story)
                span_idx = (span_start / len(data['data'][i]['story']), span_end / len(data['data'][i]['story']))

                # local_span_text = tokenize_and_encode(prev_char + data['data'][i]['answers'][idx]['span_text'] + next_char, tokenizer)
                # local_span_text = tokenize_and_encode(data['data'][i]['answers'][idx]['span_text'], tokenizer)
                # span_idx = [(i, i + len(local_span_text)) for i in range(0, len(story)) if story[i:i + len(local_span_text)] == local_span_text]

                a.append(tokenize_and_encode(data['data'][i]['answers'][idx]['input_text'], tokenizer))
                q.append(tokenize_and_encode(data['data'][i]['questions'][idx]['input_text'], tokenizer))
                span_idxs.append(span_idx)
            except:
                exc_num += 1

        dataset.append((story, q, a, span_idxs))
...
def main():
    ...
    special_tokens = ['<_STR_>', '<_END_>', '<_QUE_>', '<_ANS_>', '<_SPAN_S_>', '<_SPAN_E_>']
    ...
    token_data = load_json_dataset(args.train_dataset, tokenizer)
